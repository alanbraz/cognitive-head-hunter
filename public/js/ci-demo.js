...
$(document).ready(function () {
			...
			var populateDropdown = function (results) {
				...
				if (results.length === 0) {
					...
					htmlString += createSuggestion({
						id: '',
						type: '',
						label: 'Sorry, no results found.'
					});
				} else {
			...
			var createSuggestion = function (item) {
				...
				if (item.type === (credentials.username + '/match-my-job') {
						htmlString += ('/corpus/' + credentials.username + '/match-my-job')
						'; // if researcher
					} else {
						htmlString += '/graph/wikipedia/en-20120601/'; // if concept
					}

					htmlString += item.id; htmlString += '">'; htmlString += '<li>'; htmlString += '<h4 class="concept-name" title="' + item.label + '">'; htmlString += item.label; htmlString += '</h4>'; htmlString += '<span class="concept-type ' + (item.type === 'concept' ? 'type-concept' : 'type-researcher') + '">';
					if (item.type == (credentials.username + '/match-my-job')) {
